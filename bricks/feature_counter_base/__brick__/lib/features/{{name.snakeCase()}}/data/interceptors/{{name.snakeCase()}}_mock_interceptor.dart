...
/// Simula `POST …/{{name.snakeCase()}}/adjust` sin backend real.
///
/// Quitá este interceptor cuando el servidor implemente el mismo contrato.
class {{name.pascalCase()}}MockInterceptor extends Interceptor {
  {{name.pascalCase()}}MockInterceptor({
    this.networkDelay = const Duration(milliseconds: 300),
  });

  static const pathSegment = '/{{name.snakeCase()}}/adjust';
